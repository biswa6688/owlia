...
**BL-171**: real Claude/OpenCode brand marks (sourced from simple-icons, CC0 — fetched real SVG path data via `curl` from the raw GitHub repo, not approximated/hallucinated) replace the generic `Terminal`/`Cpu` icons in `Download.tsx`'s CLI cards and `CliPanel.tsx`'s badge/dropdown-selector/empty-state (4 separate render sites in `CliPanel.tsx` alone — `CLI_META` changed from `icon: string` (a single letter) to `Icon: ComponentType`). New `icBrand()` helper in `icons.tsx` — solid `fill="currentColor"`, no stroke, since these are filled logo marks, not the app's line-icon style. Screenshot-verified in real Edge: both logos render correctly, "Claude Code" showed "Detected (2.1.183)" confirming the real installed CLI on this machine too.

**BL-190, critical, another real user-reported bug caught live**: `GET /api/transcript/{id}` 404'd immediately after a successful analyze. `AnalyzeAsync` created `sessionId`, started the ENTIRE pipeline (including the session-row DB insert, which was step 1 *inside* `RunPipelineAsync`) on a background `Task.Run`, then returned `sessionId` to the caller right away — a classic fire-and-return-before-side-effect-lands race. The frontend's `Playground.tsx` session-restore `useEffect` fires the instant `store.sessionId` is set (immediately after `analyze()` resolves), so it very plausibly queries the transcript before the background task's first `await` even completes, especially under load (this app is loading/running large native models — ONNX Runtime, whisper.cpp, sherpa-onnx, llama.cpp — thread-pool contention makes the race window worse, not better). Fix: moved the session-row insert out of `RunPipelineAsync` and into `AnalyzeAsync` itself, `await`ed synchronously (using the already-injected scoped `_repo` — no new DI scope needed for this one write, unlike the heavy pipeline which needs its own scope to outlive the request) before `Task.Run` starts and before `sessionId` is returned. **Verified with the tightest possible race window** — upload → analyze → fetch transcript with zero artificial delay between calls, same script: now `200 {"segments":[]}`, not `404`. `GetTranscriptAsync` was already correct (only returns null/404 when the session genuinely doesn't exist, empty segments list is a normal 200) — the bug was purely about *when* the session row landed, not the read-side logic.
